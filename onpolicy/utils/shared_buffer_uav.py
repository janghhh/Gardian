...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        # ★★★★★ 2. 모든 배열의 크기를 'rollout_length' 기준으로 변경 ★★★★★
        
        # (L. 48) 'episode_length' -> 'rollout_length'
        self.share_obs = np.zeros((self.rollout_length + 1, self.n_rollout_threads, num_agents, *share_obs_shape),
                                    dtype=np.float32)
        # (L. 50) 'episode_length' -> 'rollout_length'
        self.obs = np.zeros((self.rollout_length + 1, self.n_rollout_threads, num_agents, *obs_shape), dtype=np.float32)

        # (L. 52) 'episode_length' -> 'rollout_length'
        self.rnn_states = np.zeros(
            (self.rollout_length + 1, self.n_rollout_threads, num_agents, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
        self.rnn_states_critic = np.zeros_like(self.rnn_states)

        # (L. 56) 'episode_length' -> 'rollout_length'
        self.value_preds = np.zeros(
            (self.rollout_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
        self.returns = np.zeros_like(self.value_preds)
        
        # (L. 58) 'episode_length' -> 'rollout_length'
        self.advantages = np.zeros(
            (self.rollout_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)

        if act_space.__class__.__name__ == 'Discrete':
            # (L. 63) 'episode_length' -> 'rollout_length'
            self.available_actions = np.ones((self.rollout_length + 1, self.n_rollout_threads, num_agents, act_space.n),
                                               dtype=np.float32)
        else:
        ...
        # (L. 71) 'episode_length' -> 'rollout_length'
        self.actions = np.zeros(
            (self.rollout_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        # (L. 73) 'episode_length' -> 'rollout_length'
        self.action_log_probs = np.zeros(
            (self.rollout_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        # (L. 75) 'episode_length' -> 'rollout_length'
        self.rewards = np.zeros(
            (self.rollout_length, self.n_rollout_threads, num_agents, 1), dtype=np.float32)

        # (L. 78) 'episode_length' -> 'rollout_length'
        self.masks = np.ones((self.rollout_length + 1, self.n_rollout_threads, num_agents, 1), dtype=np.float32)
