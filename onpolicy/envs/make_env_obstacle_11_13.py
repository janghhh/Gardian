...
class AirSimMultiDroneEnv:
    ...
    def __init__(
        ...
        self._threat_detected = {a: False for a in self.possible_agents}
        self._last_threat_pos = {a: np.zeros(3, dtype=np.float32) for a in self.possible_agents}
        ...
        obs_dim = 3 + 3 + self.K_nearest * (3 + 3 + 1)   # = 30
        act_dim = 2
        ...
        # 클러스터 (rel_pos(3)+rel_vel(3)+flag(1))
        low_kn  = [-200.0, -200.0, -200.0, -20.0, -20.0, -20.0, 0.0]
        high_kn = [ 200.0,  200.0,  200.0,  20.0,  20.0,  20.0, 1.0]
    ...
    def _get_knn_features(self, agent, K, now_t, all_poses):
        ...
        for _, tr in self._cluster_tracks[agent].items():
            ...
            rel_vel = tr["vel"] - vel_a
            dist = np.linalg.norm(rel_pos)
            # 🔹 DynamicObstacle과의 거리 비교
            dist_to_dynamic = np.linalg.norm(tr["centroid"][:2] - obstacle_pos[:2])
            is_dynamic = 1.0 if dist_to_dynamic < 100.0 else 0.0

            # ✅ 적군 감지 시 콘솔 출력
            if is_dynamic == 1.0:
                self._threat_detected[agent] = True
                self._last_threat_pos[agent] = tr["centroid"]
            else:
                self._threat_detected[agent] = False
                # print(f"🚨[{agent}] LiDAR에서 적군(DynamicObstacle) 감지! 거리 = {dist_to_dynamic:.2f} m")

            cand.append((dist, rel_pos, rel_vel, is_dynamic))

        cand.sort(key=lambda x: x[0])

        for i in range(K):
            if i < len(cand):
                _, rp, rv, is_dyn = cand[i]
                feats.extend([rp[0], rp[1], rp[2], rv[0], rv[1], rv[2], is_dyn])
            else:
                feats.extend([0.0]*6 + [0.0])  # padding (rel_pos+rel_vel+flag)
        return np.array(feats, dtype=np.float32)
    ...
    def _compute_reward(self, agent, all_poses):
        agent_pos   = all_poses[agent]
        leader_pos  = all_poses["Drone1"]
        obstacle_pos = all_poses[self.dynamic_name]
        ...
        for other in self.agents:
            ...
            if np.linalg.norm(agent_pos[:2] - other_pos[:2]) < 1.0:
                return -1000.0, True  # 충돌 시 종료
        ...
        # (3) 적군과의 거리 계산
        dist_guard = np.linalg.norm(agent_pos[:2] - obstacle_pos[:2])
        dist_leader_obstacle = np.linalg.norm(leader_pos[:2] - obstacle_pos[:2])
        ...
        # -------------------------
        if 5.0 <= dist <= 10.0:
            follow_reward = 10.0  # 최적 거리

        elif 0.5 <= dist < 5.0:
            # 가까운 구간: -150 → +150로 상승 (중심: 5.0m, 표준편차 σ=1.5)
            # follow_reward = -150.0 + 300.0 * math.exp(-((dist - 5.0)**2) / (2 * 1.5**2))
            follow_reward = -1000.0 + 1010.0 * math.exp(-((dist - 5.0)**2) / (2 * 1.5**2))

        elif 10.0 < dist <= 60.0:
            # 먼 구간: +150 → -150로 감소 (중심: 10.0m, 표준편차 σ=15)
            # follow_reward = -150.0 + 300.0 * math.exp(-((dist - 10.0)**2) / (2 * 15.0**2))
            follow_reward = -1000.0 + 1010.0 * math.exp(-((dist - 10.0)**2) / (2 * 15.0**2))

        # -------------------------
        # ✅ (B) 방어 보상 (5–15m 평탄 + 20m까지 선형 감쇠)
        # -------------------------
        defense_reward = 0.0

        # 🔹 항상 방어 평가
        vec_leader_obstacle = obstacle_pos[:2] - leader_pos[:2]
        vec_leader_agent = agent_pos[:2] - leader_pos[:2]

        cos_angle = np.dot(vec_leader_obstacle, vec_leader_agent) / (
            np.linalg.norm(vec_leader_obstacle) * np.linalg.norm(vec_leader_agent) + 1e-6
        )

        # ✅ (추가된 부분) 거리 기반 “사이에 있음” 조건
        dist_LA = np.linalg.norm(leader_pos[:2] - agent_pos[:2])
        dist_AO = np.linalg.norm(agent_pos[:2] - obstacle_pos[:2])
        dist_LO = np.linalg.norm(leader_pos[:2] - obstacle_pos[:2])
        in_between = abs((dist_LA + dist_AO) - dist_LO) < 3.0   # 3m 오차 허용

        # ✅ 두 조건 모두 만족해야 방어 위치로 인정
        if cos_angle > 0.5 and in_between:
            between_dist = np.linalg.norm(agent_pos[:2] - 0.5 * (leader_pos[:2] + obstacle_pos[:2]))

            # ① 5~15 m 구간: 평탄한 최대 보상
            if 5.0 <= between_dist <= 15.0:
                defense_reward += 1000.0
            # ② 15~20 m: 1000 → 0 선형 하강
            elif 15.0 < between_dist <= 20.0:
                defense_reward += 1000.0 * (1 - (between_dist - 15.0) / 5.0)
            # ③ 0~5 m: 0 → 1000 선형 상승
            elif 0.0 < between_dist < 5.0:
                defense_reward += (between_dist / 5.0) * 1000.0
            else:
                defense_reward += 0.0
        ...
        # ✅ (C) 총합 및 종료조건
        # -------------------------
        reward = follow_reward + defense_reward
    ...
    def _team_reward_and_done(self, per_agent_results, mission_accomplished=False):
        ...
        if any_fail:
            return -500.0, True, {"final_status": "FAIL_CRASH"}

        # 🛡️ 유인기가 피격당하지 않고 일정 시간 생존하면 성공
    ...
    def _update_dynamic_obstacle(self, t):
        ...
        if not hasattr(self, "_obstacle_state"):
            ...
            self._idle_pos = None
            self._next_chase_time = time.time() + random.uniform(2.0, 5.0)  # 2~5초 후 추격 시작
        ...
        if self._obstacle_state == "IDLE":
            if not hasattr(self, "_idle_start_time"):
                ...
                self._idle_wait = random.uniform(2.0, 5.0)
                print(f"[정지 대기] {self._idle_wait:.1f}s 동안 대기 후 공격 예정")

            if time.time() - self._idle_start_time >= self._idle_wait:
                self._obstacle_state = "CHASE"
                self._chase_start = time.time()
                self._chase_mode = "STRAIGHT"
                del self._idle_start_time
                print(f"[추격 시작] 리셋 위치에서 바로 공격 (모드: {self._chase_mode})")
                return
            else:
        ...
        # ------------------ CHASE 모드 ------------------
            ...
            if dist_2d <= STOP_DISTANCE:
                print(f"[리더 피격] 장애물({name})이 Drone1에 충돌 → 즉시 재배치")
                # 1️⃣ 에피소드 종료 신호용 플래그
                ...
                radius = random.uniform(60.0, 70.0)
                angle = random.uniform(0, 2*np.pi)
                new_x = lx + radius * np.cos(angle)
                new_y = ly + radius * np.sin(angle)
                new_z = self.fixed_z
                self.client.simSetVehiclePose(
                    airsim.Pose(
                        airsim.Vector3r(new_x, new_y, new_z),
    ...
    def step(self, actions):
        ...
        # dist_leader_obstacle = np.linalg.norm(leader_pos[:2] - obstacle_pos[:2])
        # print("\n--- 거리 정보 ---")
        # print(f"🟡 리더 ↔ 🔺동적장애물 거리: {dist_leader_obstacle:.2f} m")

        # for agent in self.agents:
        #     agent_pos = self._get_pose_xyz(agent)
        #     dist_leader_agent = np.linalg.norm(leader_pos[:2] - agent_pos[:2])
        #     dist_agent_obstacle = np.linalg.norm(agent_pos[:2] - obstacle_pos[:2])
        #     print(f"🔹 {agent}: 리더까지 {dist_leader_agent:.2f} m | 장애물까지 {dist_agent_obstacle:.2f} m")
        ...
        for agent in self.agents:
            agent_pos = all_poses[agent]
            if np.linalg.norm(agent_pos[:2] - obstacle_pos[:2]) < 1.0:  # 충돌 감지
                # === 위치 기반 보상 계산 ===
                vec_leader_obstacle = obstacle_pos[:2] - leader_pos[:2]
                vec_leader_agent = agent_pos[:2] - leader_pos[:2]
                ...
                # ✅ 두 조건 모두 만족해야 방어 성공으로 인정
                if cos_angle > 0.5 and in_between:
                    between_dist = np.linalg.norm(agent_pos[:2] - 0.5 * (leader_pos[:2] + obstacle_pos[:2]))

                    # ① 5~15 m 구간: 이상적 방어 위치 → 최대 보상
                    if 5.0 <= between_dist <= 15.0:
                        reward_block = 1000.0
                    # ② 15~20 m: 선형 하강 (1000 → 400)
                    elif 15.0 < between_dist <= 20.0:
                        reward_block = 1000.0 - 120.0 * (between_dist - 15.0)
                    # ③ 0~5 m: 선형 상승 (400 → 1000)
                    elif 0.0 < between_dist < 5.0:
                        reward_block = 400.0 + 120.0 * between_dist
                    # ④ 나머지: 비효율적 위치
                    else:
                        reward_block = 200.0
                else:
                    # 방향이 다르거나 사이가 아닐 경우 → 방어 불량
                    reward_block = 100.0

                print(f"🛡️[가디언 차폐 성공] {agent} ↔ DynamicObstacle ({reward_block:.1f})점")

                # 팀 전체 동일 보상 + 에피소드 계속
                rewards_list = [reward_block] * len(self.agents)
                dones_list = [False] * len(self.agents)
                ...
                infos_list = [{
                    "event": "guardian_block_continue",
                    "blocker": agent,
                    "block_reward": reward_block
